...
def populate_db():
    ...
    def create_users_table(User, UserRole, Card, Address):
        ...
        click.echo("populating employees and employees's role tables")
        for worker in complete_admin_populate_data:
            user_ = User(full_name=worker.get("full_name"), email=worker.get(
                "email"), password=worker.get("password"), registration_date=worker.get("registration_date"))
            user_.create()
            user_.save()
            userrole_ = UserRole(user_id=user_.id, role_id=worker.get("role_id"))
            userrole_.create()
            userrole_.save()
        click.echo("done populating employees and employees's role tables \n")
        ...
        for all_user_info in complete_user_populate_data:
            ...
            user_ = User(full_name=user_info.get("full_name"), email=user_info.get(
                "email"), password=user_info.get("password"), registration_date=user_info.get("registration_date"))
            user_.create()
            user_.save()

            userrole_ = UserRole(user_id=user_.id, role_id=user_info.get("role_id"))
            userrole_.create()
            userrole_.save()
            ...
            for user_card in all_user_info[1]:
                ...
                if user_card.get("card_exp_date") > datetime.date.today():
                    card_ = Card(user_id=user_.id, card_number=user_card.get("card_number"), expiration_date=user_card.get(
                        "card_exp_date"), cvv=user_card.get("cvv"), holder_name=user_card.get("holder_name"))
                else:
                    card_ = Card(user_id=user_.id, card_number=user_card.get("card_number"), expiration_date=user_card.get(
                        "card_exp_date"), cvv=user_card.get("cvv"), holder_name=user_card.get("holder_name"), usable=False)
                card_.create()
            ...
            for user_address in all_user_info[2]:
                address_ = Address(user_id=user_.id, full_name=user_address.get("full_name"), mobile_number=user_address.get("mobile_number"), country_id=user_address.get("country_id"), city=user_address.get(
